...
async def command_set(update: Update, context: CallbackContext, session_id):
    args = context.args
    if not args:
        await update.message.reply_text("⚠️Specify what to set (model, temperature, system_prompt, max_tokens, openai_api_key).")
        return
...
{model_list}")
    ...
    if preference == 'openai_api_key':
        ...
        if value.isdigit():
            ...
            await update.message.reply_text("⚠️Invalid max tokens.")
    else:
        await update.message.reply_text("⚠️Invalid setting.")
...
"
    if not session_data:
        message += "Session data not initialized.
"
    else:
...
"
    for cmd, desc in [("/reset", "Reset settings"), ("/clear", "Clear chat history"),
                      ("/set", "Change settings"), ("/show", "Show session data"),
                      ("/help", "Command list")]:
