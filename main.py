...
async def handle_message(update: Update, context: CallbackContext, session_id):
    ...
    messages_for_api = [msg for msg in session_data['chat_history']]
...
async def command_set(update: Update, context: CallbackContext, session_id):
    args = context.args
    if not args:
        await update.message.reply_text("⚠️Please specify what to set (model, temperature, max_tokens, openai_api_key).")
        return
    ...
    if preference == 'temperature':
        try:
            t = float(value)
            update_session_preference(session_id, 'temperature', t)
            await update.message.reply_text(f"✅Temperature: {{t}}")
        except:
            await update.message.reply_text("⚠️Invalid temperature.")
        return
    if preference == 'max_tokens' and value.isdigit():
        update_session_preference(session_id, 'max_tokens', int(value))
        await update.message.reply_text(f"✅Max tokens: {{value}}")
        return
    await update.message.reply_text("⚠️Unknown setting.")

@get_session_id
async def command_show(update: Update, context: CallbackContext, session_id):
    session_data = SESSION_DATA.get(session_id, {{}})
    text = f"**Session Info:**\nModel: {{session_data.get('model')}}\nTemp: {{session_data.get('temperature')}}\nTokens: {{session_data.get('max_tokens')}}\n\n"
