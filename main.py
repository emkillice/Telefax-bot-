...
async def handle_message(update: Update, context: CallbackContext, session_id):
    ...
    # Лимит на 3 сообщения
    if user_id not in UNLIMITED_USERS:
        USER_COUNTER[user_id] = USER_COUNTER.get(user_id, 0) + 1
        if USER_COUNTER[user_id] > MESSAGE_LIMIT:
            await update.message.reply_text("Вы использовали 3 бесплатных запроса. Чтобы продолжить, оформите подписку: https://boosty.to/ваш_бот")
            return
    ...
    if isinstance(response, str) and response.strip():
        ...
        await update.message.reply_text("⚠️ Ошибка при генерации ответа. Попробуйте позже.")
