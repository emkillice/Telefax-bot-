...
async def command_set(update: Update, context: CallbackContext, session_id):
    args = context.args
    if not args:
        await update.message.reply_text("⚠️Usage: /set [model|temperature|system_prompt|max_tokens|openai_api_key] value")
        return
    ...
    if preference == 'model':
        if value in sum(VALID_MODELS.values(), []):
            ...
            await update.message.reply_text("⚠️ Invalid model name.")
        ...
        try:
            ...
            await update.message.reply_text("✅ Temperature set.")
        except:
            await update.message.reply_text("⚠️ Invalid temperature value.")
    elif preference == 'max_tokens':
        if value.isdigit():
            SESSION_DATA[session_id]['max_tokens'] = int(value)
            await update.message.reply_text("✅ Max tokens set.")
        else:
            await update.message.reply_text("⚠️ Must be a number.")

@get_session_id
async def command_show(update: Update, context: CallbackContext, session_id):
    session_data = SESSION_DATA.get(session_id, {})
    await update.message.reply_text(str(session_data))

async def command_help(update: Update, context: CallbackContext):
    help_text = "/start /reset /clear /set /show /help"
